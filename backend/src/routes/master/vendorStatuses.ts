...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    let countQuery = `SELECT COUNT(*) FROM vendor_statuses WHERE deleted_at IS NULL`;
    const params: any[] = [];
    ...
    sendError(res, 'SERVER_ERROR', 'Failed to fetch vendor statuses', 500);
  }
});

router.get('/:id', async (req: Request, res: Response) => {
  try {
    const result = await pool.query('SELECT * FROM vendor_statuses WHERE id=$1 AND deleted_at IS NULL', [req.params.id]);
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const dup = await pool.query('SELECT id FROM vendor_statuses WHERE code=$1 AND company_id=$2 AND deleted_at IS NULL', [code, companyId]);
    ...
    const result = await pool.query(
      ...
      [companyId, code, name, name_ar||null, description||null, description_ar||null, color||null, allows_purchase_orders||false, allows_invoices||false, allows_payments||false, is_default||false, sort_order||0, is_active]
    );
...
router.put('/:id', async (req: Request, res: Response) => {
  try {
    ...
    const existing = await pool.query('SELECT * FROM vendor_statuses WHERE id=$1 AND deleted_at IS NULL', [req.params.id]);
    ...
    const result = await pool.query(
      `UPDATE vendor_statuses SET code=COALESCE($1,code), name=COALESCE($2,name), name_ar=COALESCE($3,name_ar),
       description=COALESCE($4,description), description_ar=COALESCE($5,description_ar), color=COALESCE($6,color),
      ...
      [code, name, name_ar, description, description_ar, color, allows_purchase_orders, allows_invoices, allows_payments, is_default, sort_order, is_active, req.params.id]
    );
...
router.delete('/:id', async (req: Request, res: Response) => {
  try {
    const result = await pool.query('UPDATE vendor_statuses SET deleted_at=NOW() WHERE id=$1 AND deleted_at IS NULL RETURNING id', [req.params.id]);
    if (result.rows.length === 0) return sendError(res, 'NOT_FOUND', 'Not found', 404);
