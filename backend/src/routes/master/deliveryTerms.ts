...
router.get('/', authenticate, async (req, res) => {
  try {
    ...
    const countResult = await pool.query(`SELECT COUNT(*) FROM delivery_terms WHERE deleted_at IS NULL`);
...
router.get('/:id', authenticate, async (req, res) => {
  try {
    ...
    if (result.rows.length === 0) {
      return sendError(res, 'NOT_FOUND', 'delivery term not found', 404);
    }
