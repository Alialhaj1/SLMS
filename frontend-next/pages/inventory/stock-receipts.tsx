...
function StockReceiptsPage() {
  ...
  return (
    <MainLayout>
      <Head>
        <title>{tr('menu.advancedWarehouses.stockReceipts', 'Stock Receipts')} - SLMS</title>
      </Head>

      <div className="p-6">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
            {tr('menu.advancedWarehouses.stockReceipts', 'Stock Receipts')}
          </h1>
              ...
              <table className="min-w-full">
                <thead className="bg-gray-50 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-600">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-right text-xs font-medium text-gray-700 dark:text-gray-300 uppercase">{tr('inventory.unitCost', 'Unit Cost')}</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                  {rows.map((r) => (
                    <tr key={r.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                      ...
                      <td className="px-6 py-4 text-sm text-gray-900 dark:text-white text-right">{Number(r.qty_delta).toFixed(4)}</td>
                      <td className="px-6 py-4 text-sm text-gray-900 dark:text-white text-right">{Number(r.unit_cost || 0).toFixed(4)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
...
export default withPermission(MenuPermissions.Warehouses.InventoryOperations.Receipts.View, StockReceiptsPage);
