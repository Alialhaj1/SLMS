...
function StockReturnsPage() {
  ...
  const fetchReturns = async () => {
    ...
    try {
      ...
      const resp = await fetch(`${apiUrl}/api/inventory/returns`, {
        headers: {
  ...
  const directionLabel = (txnType: string) => {
    if (txnType === 'return_out') return locale === 'ar' ? 'خروج' : 'Out';
  ...
  return (
    <MainLayout>
      <Head>
        <title>{tr('menu.advancedWarehouses.stockReturns', 'Stock Returns')} - SLMS</title>
      </Head>

      <div className="p-6">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
            {tr('menu.advancedWarehouses.stockReturns', 'Stock Returns')}
          </h1>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1">
              ...
              >
                <option value="in">{locale === 'ar' ? 'مرتجع دخول' : 'Return In'}</option>
                <option value="out">{locale === 'ar' ? 'مرتجع خروج' : 'Return Out'}</option>
              </select>
            </div>
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                  {rows.map((r) => (
                    <tr key={r.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                      ...
                      <td className="px-6 py-4 text-sm text-gray-700 dark:text-gray-300">{directionLabel(r.txn_type)}</td>
...
export default withPermission(MenuPermissions.Warehouses.InventoryOperations.Returns.View, StockReturnsPage);
