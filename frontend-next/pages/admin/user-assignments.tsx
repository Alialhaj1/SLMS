...
export default function UserAssignmentsPage() {
  ...
  const fetchAssignments = async () => {
    ...
    try {
      ...
      showToast('error', t('errors.load') || 'Failed to load assignments');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAssign = async () => {
    if (!formUserId || !formCompanyId) { showToast('error', 'User and company are required'); return; }
    setSaving(true);
    try {
      const token = localStorage.getItem('accessToken');
      const res = await fetch('http://localhost:4000/api/user-company-roles', {
  ...
  return (
    <MainLayout>
      <Head><title>{t('userAssignments.title') || 'User Assignments'} - SLMS</title></Head>
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
              <UserGroupIcon className="w-7 h-7 text-blue-500" />
              {t('userAssignments.title') || 'User-Company Role Assignments'}
            </h1>
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{t('userAssignments.subtitle') || 'Map users to companies and assign roles'}</p>
          </div>
          <button onClick={() => setShowForm(!showForm)} className="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm font-medium transition-colors">
            <PlusIcon className="w-4 h-4" /> Assign User
          </button>
        </div>
        ...
        {showForm && (
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-6 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <input value={formUserId} onChange={(e) => setFormUserId(e.target.value)} placeholder="User ID" type="number" className="px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-900 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-blue-500" />
              <input value={formCompanyId} onChange={(e) => setFormCompanyId(e.target.value)} placeholder="Company ID" type="number" className="px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-900 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-blue-500" />
            ...
            <div className="flex gap-2">
              <button onClick={handleAssign} disabled={saving} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm font-medium disabled:opacity-50">{saving ? 'Saving...' : 'Assign'}</button>
        ...
        <div className="relative max-w-md">
          ...
          <input type="text" placeholder="Search assignments..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-gray-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-gray-900 dark:text-white placeholder-gray-400 focus:ring-2 focus:ring-blue-500 text-sm" />
        </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 dark:bg-slate-900/50">
              <tr>
                {['User', 'Company', 'Role', 'Assigned', 'Actions'].map((h) => (
              ...
              {loading ? Array.from({ length: 4 }).map((_, i) => <SkeletonRow key={i} />) : filtered.length === 0 ? (
                ...
                <tr key={a.id} className="hover:bg-gray-50 dark:hover:bg-slate-700/30 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <button onClick={() => handleRemove(a.id)} disabled={removing === a.id} className="text-red-500 hover:text-red-700 disabled:opacity-50" title="Remove">
