...
const mockStatuses: ClearanceStatusItem[] = [
  { id: 1, code: 'PENDING_DOCS', name: 'Pending Documents', nameAr: 'بانتظار المستندات', color: 'amber', isTerminal: false, status: 'active' },
  { id: 2, code: 'UNDER_REVIEW', name: 'Under Review', nameAr: 'قيد المراجعة', color: 'blue', isTerminal: false, status: 'active' },
  { id: 3, code: 'CLEARED', name: 'Cleared', nameAr: 'مخلّص', color: 'green', isTerminal: true, status: 'active' },
  { id: 4, code: 'REJECTED', name: 'Rejected', nameAr: 'مرفوض', color: 'red', isTerminal: true, status: 'active' },
  { id: 5, code: 'ARCHIVED', name: 'Archived', nameAr: 'مؤرشف', color: 'gray', isTerminal: true, status: 'inactive' },
];
...
export default function ClearanceStatusPage() {
  ...
  const canView = hasAnyPermission([MenuPermissions.MasterData.ClearanceStatus.View]);
  const canCreate = hasAnyPermission([MenuPermissions.MasterData.ClearanceStatus.Create]);
  const canEdit = hasAnyPermission([MenuPermissions.MasterData.ClearanceStatus.Edit]);

  const [items] = useState<ClearanceStatusItem[]>(mockStatuses);
  const [search, setSearch] = useState('');
  const [status, setStatus] = useState<'all' | ActiveStatus>('all');
  const [selected, setSelected] = useState<ClearanceStatusItem | null>(null);
  ...
  const [formData, setFormData] = useState({
    ...
    name: '',
    nameAr: '',
    color: 'blue' as ClearanceStatusItem['color'],
    isTerminal: false,
    status: 'active' as ActiveStatus,
  });

  const colorPill = (c: ClearanceStatusItem['color']) => {
    const styles: Record<ClearanceStatusItem['color'], string> = {
    ...
    const labels: Record<ClearanceStatusItem['color'], { en: string; ar: string }> = {
    ...
    return (
      <span className={clsx('px-2 py-0.5 text-xs font-medium rounded-full', styles[c])}>
        {locale === 'ar' ? labels[c].ar : labels[c].en}
      </span>
    );
  };

  const activeBadge = (s: ActiveStatus) => {
  ...
  const terminalCount = items.filter((i) => i.isTerminal).length;
  ...
  const handleCreate = () => {
    ...
    setCreateOpen(false);
    setFormData({ code: '', name: '', nameAr: '', color: 'blue', isTerminal: false, status: 'active' });
  };
  ...
  if (!canView) {
    return (
      <MainLayout>
        <Head>
          <title>{locale === 'ar' ? 'حالات التخليص - SLMS' : 'Clearance Status - SLMS'}</title>
        </Head>
        <div className="text-center py-12">
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-2">{locale === 'ar' ? 'لا تملك صلاحية عرض حالات التخليص.' : "You don't have permission to view clearance statuses."}</p>
        </div>
      </MainLayout>
    );
  }
    ...
    <MainLayout>
      ...
      <div className="space-y-6 animate-fade-in">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-slate-100 dark:bg-slate-900/30 rounded-lg">
              <FlagIcon className="h-6 w-6 text-slate-700 dark:text-slate-300" />
            </div>
            <div>
              <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{locale === 'ar' ? 'حالات التخليص' : 'Clearance Status'}</h1>
              <p className="text-sm text-gray-500 dark:text-gray-400">{locale === 'ar' ? 'تعريف حالات التخليص المستخدمة في سير العمل' : 'Define clearance workflow statuses'}</p>
            </div>
          </div>
          {canCreate && (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-4">
            <p className="text-sm text-gray-500 dark:text-gray-400">{locale === 'ar' ? 'نهائية' : 'Terminal'}</p>
            <p className="text-2xl font-bold text-gray-900 dark:text-white">{terminalCount}</p>
          </div>
        </div>
            ...
            <table className="w-full">
              <thead className="bg-gray-50 dark:bg-gray-700">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{locale === 'ar' ? 'اللون' : 'Color'}</th>
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                {filtered.map((i) => (
                  <tr key={i.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                    ...
                    <td className="px-4 py-3">{colorPill(i.color)}</td>
                    <td className="px-4 py-3 text-gray-500">{i.isTerminal ? (locale === 'ar' ? 'نعم' : 'Yes') : (locale === 'ar' ? 'لا' : 'No')}</td>
                    <td className="px-4 py-3">{activeBadge(i.status)}</td>
        ...
        {selected && (
          <div className="space-y-4">
            <div className="flex items-start justify-between gap-3">
              ...
              {activeBadge(selected.status)}
            </div>

            <div className="grid grid-cols-2 gap-4">
              <div className="p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                <p className="text-xs text-gray-500">{locale === 'ar' ? 'اللون' : 'Color'}</p>
                <div className="mt-1">{colorPill(selected.color)}</div>
              </div>
              <div className="p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
      ...
      <Modal isOpen={canCreate && createOpen} onClose={() => setCreateOpen(false)} title={locale === 'ar' ? 'حالة تخليص جديدة' : 'New Clearance Status'} size="lg">
        <div className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select value={formData.color} onChange={(e) => setFormData({ ...formData, color: e.target.value as any })} className="input">
                <option value="gray">{locale === 'ar' ? 'رمادي' : 'Gray'}</option>
                <option value="blue">{locale === 'ar' ? 'أزرق' : 'Blue'}</option>
                <option value="amber">{locale === 'ar' ? 'كهرماني' : 'Amber'}</option>
