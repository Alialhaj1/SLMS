...
export default function DeferredPoliciesPage() {
  const { user } = useAuth();
  const { locale } = useLocale();
  const { showToast } = useToast();
  const [items, setItems] = useState<DeferredPolicy[]>([]);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await fetch('http://localhost:4000/api/master/deferred-policies', {
  ...
  const typeBadge = (t: string) => {
    ...
    return <span className={`px-2 py-0.5 text-xs font-medium rounded-full ${cls}`}>{t}</span>;
  };

  return (
    <MainLayout>
      <Head><title>{locale === 'ar' ? 'سياسات الإيرادات/المصروفات المؤجلة - SLMS' : 'Deferred Policies - SLMS'}</title></Head>
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
              {locale === 'ar' ? 'سياسات الإيرادات/المصروفات المؤجلة' : 'Deferred Revenue/Expense Policies'}
            </h1>
            <p className="text-sm text-gray-500 dark:text-gray-400">
              {locale === 'ar' ? 'إدارة سياسات الاعتراف بالإيرادات والمصروفات' : 'Manage revenue and expense recognition policies'}
            </p>
          </div>
          ...
          {loading ? (
            ...
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              <thead className="bg-gray-50 dark:bg-gray-900/50">
                <tr>
                  {['Name', 'Type', 'Period (Months)', 'Recognition Method', 'Status', 'Actions'].map(h => (
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                {filtered.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-300">{item.recognition_method}</td>
