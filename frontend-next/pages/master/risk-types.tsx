...
import { ExclamationTriangleIcon, EyeIcon, PlusIcon } from '@heroicons/react/24/outline';
...
const mockTypes: RiskType[] = [
  { id: 1, code: 'OPR', nameEn: 'Operational Risk', nameAr: 'مخاطر تشغيلية', isActive: true },
  { id: 2, code: 'FIN', nameEn: 'Financial Risk', nameAr: 'مخاطر مالية', isActive: true },
  { id: 3, code: 'CMP', nameEn: 'Compliance Risk', nameAr: 'مخاطر امتثال', isActive: true },
];
...
export default function RiskTypesPage() {
  ...
  const canView = hasAnyPermission([MenuPermissions.MasterData.RiskTypes.View]);
  const canCreate = hasAnyPermission([MenuPermissions.MasterData.RiskTypes.Create]);

  const [items] = useState<RiskType[]>(mockTypes);
  ...
  if (!canView) {
    return (
      <MainLayout>
        <Head>
          <title>{locale === 'ar' ? 'أنواع المخاطر - SLMS' : 'Risk Types - SLMS'}</title>
        </Head>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
                {locale === 'ar' ? 'أنواع المخاطر' : 'Risk Types'}
              </h1>
              <p className="text-sm text-gray-500 dark:text-gray-400">
                {locale === 'ar' ? 'تصنيف المخاطر' : 'Risk classification'}
              </p>
            </div>
          </div>
