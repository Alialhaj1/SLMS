...
const mockTypes: ReportType[] = [
  { id: 1, code: 'FIN', nameEn: 'Financial Report', nameAr: 'تقرير مالي', isActive: true },
  { id: 2, code: 'OPS', nameEn: 'Operational Report', nameAr: 'تقرير تشغيلي', isActive: true },
  { id: 3, code: 'AUD', nameEn: 'Audit Report', nameAr: 'تقرير تدقيق', isActive: true },
];
...
export default function ReportTypesPage() {
  ...
  const canView = hasAnyPermission([MenuPermissions.MasterData.ReportTypes.View]);
  const canCreate = hasAnyPermission([MenuPermissions.MasterData.ReportTypes.Create]);

  const [items] = useState<ReportType[]>(mockTypes);
  ...
  if (!canView) {
    return (
      <MainLayout>
        <Head>
          <title>{locale === 'ar' ? 'أنواع التقارير - SLMS' : 'Report Types - SLMS'}</title>
        </Head>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">
                {locale === 'ar' ? 'تصنيف التقارير' : 'Report classification'}
              </p>
            </div>
          </div>
