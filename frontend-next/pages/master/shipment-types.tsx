...
const mockTypes: ShipmentType[] = [
  { id: 1, code: 'FCL', nameEn: 'Full Container Load', nameAr: 'حاوية كاملة', isActive: true },
  { id: 2, code: 'LCL', nameEn: 'Less than Container Load', nameAr: 'أقل من حاوية', isActive: true },
  { id: 3, code: 'EXP', nameEn: 'Express Shipment', nameAr: 'شحنة سريعة', isActive: true },
];
...
export default function ShipmentTypesPage() {
  ...
  const canView = hasAnyPermission([MenuPermissions.MasterData.ShipmentTypes.View]);
  const canCreate = hasAnyPermission([MenuPermissions.MasterData.ShipmentTypes.Create]);

  const [items] = useState<ShipmentType[]>(mockTypes);
  ...
  if (!canView) {
    return (
      <MainLayout>
        <Head>
          <title>{locale === 'ar' ? 'أنواع الشحنات - SLMS' : 'Shipment Types - SLMS'}</title>
        </Head>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-cyan-100 dark:bg-cyan-900/30 rounded-lg">
              <TruckIcon className="h-6 w-6 text-cyan-600 dark:text-cyan-300" />
            </div>
            <div>
              <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
                {locale === 'ar' ? 'أنواع الشحنات' : 'Shipment Types'}
              </h1>
              <p className="text-sm text-gray-500 dark:text-gray-400">
                {locale === 'ar' ? 'تصنيف الشحنات' : 'Shipment classification'}
              </p>
            </div>
          </div>
