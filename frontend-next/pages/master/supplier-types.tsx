...
export default function SupplierTypesPage() {
  ...
  const [items, setItems] = useState<SupplierType[]>([]);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await fetch('http://localhost:4000/api/master/supplier-types', {
  ...
  return (
    <MainLayout>
      <Head><title>{locale === 'ar' ? 'أنواع الموردين - SLMS' : 'Supplier Types - SLMS'}</title></Head>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
                {locale === 'ar' ? 'أنواع الموردين' : 'Supplier Types'}
              </h1>
              <p className="text-sm text-gray-500 dark:text-gray-400">
                {locale === 'ar' ? 'إدارة تصنيفات أنواع الموردين' : 'Manage supplier type classifications'}
              </p>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              <thead className="bg-gray-50 dark:bg-gray-900/50">
                <tr>
                  {['Code', 'Name', 'Description', 'Suppliers', 'Status', 'Actions'].map(h => (
