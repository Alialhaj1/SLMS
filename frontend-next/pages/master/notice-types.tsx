...
interface NoticeType { id: number; code: string; nameEn: string; nameAr: string; isActive: boolean; }

const mockTypes: NoticeType[] = [
  { id: 1, code: 'SYSTEM', nameEn: 'System Notice', nameAr: 'إشعار نظام', isActive: true },
  { id: 2, code: 'PAYMENT', nameEn: 'Payment Notice', nameAr: 'إشعار دفع', isActive: true },
  { id: 3, code: 'SHIPMENT', nameEn: 'Shipment Notice', nameAr: 'إشعار شحن', isActive: true },
];
...
export default function NoticeTypesPage() {
  ...
  const canView = hasAnyPermission([MenuPermissions.MasterData.NoticeTypes.View]);
  const canCreate = hasAnyPermission([MenuPermissions.MasterData.NoticeTypes.Create]);

  const [items] = useState<NoticeType[]>(mockTypes);
  ...
  if (!canView) {
    return (
      <MainLayout>
        <Head><title>{locale === 'ar' ? 'أنواع الإشعارات - SLMS' : 'Notice Types - SLMS'}</title></Head>
        <div className="text-center py-12"><BellIcon className="w-16 h-16 mx-auto text-gray-400 mb-4" /><h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100">{locale === 'ar' ? 'غير مصرح' : 'Access Denied'}</h2></div>
      </MainLayout>
    );
  }
    ...
    <MainLayout>
      ...
      <div className="space-y-6 animate-fade-in">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-3"><div className="p-2 bg-yellow-100 dark:bg-yellow-900/30 rounded-lg"><BellIcon className="h-6 w-6 text-yellow-600 dark:text-yellow-300" /></div><div><h1 className="text-2xl font-bold text-gray-900 dark:text-white">{locale === 'ar' ? 'أنواع الإشعارات' : 'Notice Types'}</h1><p className="text-sm text-gray-500 dark:text-gray-400">{locale === 'ar' ? 'أنواع الإشعارات والتنبيهات' : 'Notice and alert types'}</p></div></div>
