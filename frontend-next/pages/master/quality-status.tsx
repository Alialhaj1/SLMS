...
import { CheckBadgeIcon, EyeIcon, PlusIcon } from '@heroicons/react/24/outline';
...
const mockStatuses: QualityStatus[] = [
  { id: 1, code: 'PASSED', nameEn: 'Passed', nameAr: 'ناجح', isActive: true },
  { id: 2, code: 'FAILED', nameEn: 'Failed', nameAr: 'فاشل', isActive: true },
  { id: 3, code: 'ON_HOLD', nameEn: 'On Hold', nameAr: 'موقوف', isActive: true },
];
...
export default function QualityStatusPage() {
  ...
  const canView = hasAnyPermission([MenuPermissions.MasterData.QualityStatus.View]);
  const canCreate = hasAnyPermission([MenuPermissions.MasterData.QualityStatus.Create]);

  const [items] = useState<QualityStatus[]>(mockStatuses);
  const [search, setSearch] = useState('');
  const [selected, setSelected] = useState<QualityStatus | null>(null);
  ...
  if (!canView) {
    return (
      <MainLayout>
        <Head>
          <title>{locale === 'ar' ? 'حالات الجودة - SLMS' : 'Quality Status - SLMS'}</title>
        </Head>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-rose-100 dark:bg-rose-900/30 rounded-lg">
              <CheckBadgeIcon className="h-6 w-6 text-rose-600 dark:text-rose-300" />
            </div>
            <div>
              <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
                {locale === 'ar' ? 'حالات الجودة' : 'Quality Status'}
              </h1>
              <p className="text-sm text-gray-500 dark:text-gray-400">
                {locale === 'ar' ? 'نتائج الفحص والجودة' : 'Inspection and quality outcomes'}
              </p>
            </div>
          </div>
