...
type InsuranceTypeStatus = 'active' | 'inactive';
type InsuranceCategory = 'cargo' | 'liability' | 'vehicle' | 'other';
...
interface InsuranceType {
  ...
  status: InsuranceTypeStatus;
}

const mockInsuranceTypes: InsuranceType[] = [
  { id: 1, code: 'CARGO', name: 'Cargo Insurance', nameAr: 'تأمين البضائع', category: 'cargo', status: 'active' },
  { id: 2, code: 'LIAB', name: 'Liability Insurance', nameAr: 'تأمين المسؤولية', category: 'liability', status: 'active' },
  { id: 3, code: 'VEH', name: 'Vehicle Insurance', nameAr: 'تأمين المركبات', category: 'vehicle', status: 'inactive' },
];
...
export default function InsuranceTypesPage() {
  ...
  const canView = hasAnyPermission([MenuPermissions.Master.View, MenuPermissions.MasterData.InsuranceTypes.View]);
  const canCreate = hasAnyPermission([MenuPermissions.Master.Create, MenuPermissions.MasterData.InsuranceTypes.Create]);
  const canEdit = hasAnyPermission([MenuPermissions.Master.Edit, MenuPermissions.MasterData.InsuranceTypes.Edit]);

  const [items] = useState<InsuranceType[]>(mockInsuranceTypes);
  const [selectedStatus, setSelectedStatus] = useState<'all' | InsuranceTypeStatus>('all');
  ...
  const [formData, setFormData] = useState({
    ...
    name: '',
    nameAr: '',
    category: 'cargo' as InsuranceCategory,
    status: 'active' as InsuranceTypeStatus,
  });
  ...
  const categoryLabel = (category: InsuranceCategory) => {
    const labels: Record<InsuranceCategory, { en: string; ar: string }> = {
  ...
  const getStatusBadge = (status: InsuranceTypeStatus) => {
    const styles: Record<InsuranceTypeStatus, string> = {
      active: 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300',
      inactive: 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300',
    };
    const labels: Record<InsuranceTypeStatus, { en: string; ar: string }> = {
  ...
  const handleCreate = () => {
    ...
    setCreateOpen(false);
    setFormData({ code: '', name: '', nameAr: '', category: 'cargo', status: 'active' });
  };
  ...
  if (!canView) {
    return (
      <MainLayout>
        <Head>
          <title>{locale === 'ar' ? 'أنواع التأمين - SLMS' : 'Insurance Types - SLMS'}</title>
        </Head>
        <div className="text-center py-12">
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-2">
            {locale === 'ar' ? 'لا تملك صلاحية عرض أنواع التأمين.' : "You don't have permission to view insurance types."}
          </p>
        </div>
      </MainLayout>
    );
  ...
  return (
    <MainLayout>
      ...
      <div className="space-y-6 animate-fade-in">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{locale === 'ar' ? 'أنواع التأمين' : 'Insurance Types'}</h1>
              <p className="text-sm text-gray-500 dark:text-gray-400">{locale === 'ar' ? 'تعريف أنواع التأمين المستخدمة في الشحن' : 'Define insurance types used across shipments'}</p>
            </div>
          </div>
          {canCreate && (
      ...
      <Modal isOpen={canCreate && createOpen} onClose={() => setCreateOpen(false)} title={locale === 'ar' ? 'نوع تأمين جديد' : 'New Insurance Type'} size="lg">
        <div className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Input label={locale === 'ar' ? 'الكود' : 'Code'} value={formData.code} onChange={(e) => setFormData({ ...formData, code: e.target.value })} placeholder="CARGO" />
            <div>
