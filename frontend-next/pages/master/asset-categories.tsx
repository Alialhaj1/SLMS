...
type AssetCategoryStatus = 'active' | 'inactive';
...
const mockCategories: AssetCategory[] = [
  { id: 1, code: 'VEH', name: 'Vehicles', nameAr: 'مركبات', depreciationYears: 5, status: 'active' },
  { id: 2, code: 'EQP', name: 'Equipment', nameAr: 'معدات', depreciationYears: 4, status: 'active' },
  { id: 3, code: 'IT', name: 'IT Assets', nameAr: 'أصول تقنية', depreciationYears: 3, status: 'inactive' },
];
...
export default function AssetCategoriesPage() {
  ...
  const canView = hasAnyPermission([MenuPermissions.Master.View, MenuPermissions.MasterData.AssetCategories.View]);
  const canCreate = hasAnyPermission([MenuPermissions.Master.Create, MenuPermissions.MasterData.AssetCategories.Create]);
  const canEdit = hasAnyPermission([MenuPermissions.Master.Edit, MenuPermissions.MasterData.AssetCategories.Edit]);

  const [items] = useState<AssetCategory[]>(mockCategories);
  const [selectedStatus, setSelectedStatus] = useState<'all' | AssetCategoryStatus>('all');
  ...
  const [formData, setFormData] = useState({
    ...
    nameAr: '',
    depreciationYears: '5',
    status: 'active' as AssetCategoryStatus,
  });
  ...
  const avgDep = items.length ? Math.round((items.reduce((s, i) => s + i.depreciationYears, 0) / items.length) * 10) / 10 : 0;

  const getStatusBadge = (status: AssetCategoryStatus) => {
    const styles: Record<AssetCategoryStatus, string> = {
      active: 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300',
      inactive: 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300',
    };
    const labels: Record<AssetCategoryStatus, { en: string; ar: string }> = {
  ...
  const handleCreate = () => {
    ...
    setCreateOpen(false);
    setFormData({ code: '', name: '', nameAr: '', depreciationYears: '5', status: 'active' });
  };
  ...
  if (!canView) {
    return (
      <MainLayout>
        <Head>
          <title>{locale === 'ar' ? 'فئات الأصول - SLMS' : 'Asset Categories - SLMS'}</title>
        </Head>
        <div className="text-center py-12">
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-2">{locale === 'ar' ? 'لا تملك صلاحية عرض فئات الأصول.' : "You don't have permission to view asset categories."}</p>
        </div>
      </MainLayout>
    );
  }
  ...
  return (
    <MainLayout>
      ...
      <div className="space-y-6 animate-fade-in">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{locale === 'ar' ? 'فئات الأصول' : 'Asset Categories'}</h1>
              <p className="text-sm text-gray-500 dark:text-gray-400">{locale === 'ar' ? 'تصنيف الأصول وسياسات الإهلاك' : 'Classify assets and depreciation policies'}</p>
            </div>
          </div>
          {canCreate && (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-4">
            <p className="text-sm text-gray-500 dark:text-gray-400">{locale === 'ar' ? 'متوسط الإهلاك (سنوات)' : 'Avg. Depreciation (yrs)'}</p>
            <p className="text-2xl font-bold text-gray-900 dark:text-white">{avgDep}</p>
          </div>
        </div>
            ...
            <table className="w-full">
              <thead className="bg-gray-50 dark:bg-gray-700">
                <tr>
                  ...
                  <th className="px-4 py-3 text-center text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{locale === 'ar' ? 'سنوات الإهلاك' : 'Depreciation (yrs)'}</th>
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                {filtered.map((i) => (
                  <tr key={i.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                    ...
                    <td className="px-4 py-3 text-center text-gray-900 dark:text-white">{i.depreciationYears}</td>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                <p className="text-xs text-gray-500">{locale === 'ar' ? 'سنوات الإهلاك' : 'Depreciation Years'}</p>
                <p className="font-medium text-gray-900 dark:text-white">{selected.depreciationYears}</p>
              </div>
              <div className="p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
      ...
      <Modal isOpen={canCreate && createOpen} onClose={() => setCreateOpen(false)} title={locale === 'ar' ? 'فئة أصل جديدة' : 'New Asset Category'} size="lg">
        <div className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Input label={locale === 'ar' ? 'الكود' : 'Code'} value={formData.code} onChange={(e) => setFormData({ ...formData, code: e.target.value })} placeholder="VEH" />
            <Input label={locale === 'ar' ? 'سنوات الإهلاك' : 'Depreciation Years'} value={formData.depreciationYears} onChange={(e) => setFormData({ ...formData, depreciationYears: e.target.value })} placeholder="5" />
