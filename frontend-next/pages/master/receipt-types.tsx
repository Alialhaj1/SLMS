...
import { BanknotesIcon, EyeIcon, PlusIcon } from '@heroicons/react/24/outline';
...
const mockTypes: ReceiptType[] = [
  { id: 1, code: 'CASH', nameEn: 'Cash Receipt', nameAr: 'سند قبض نقدي', isActive: true },
  { id: 2, code: 'BANK', nameEn: 'Bank Receipt', nameAr: 'سند قبض بنكي', isActive: true },
  { id: 3, code: 'ADV', nameEn: 'Advance Receipt', nameAr: 'سند قبض مقدم', isActive: true },
];
...
export default function ReceiptTypesPage() {
  ...
  const canView = hasAnyPermission([MenuPermissions.MasterData.ReceiptTypes.View]);
  const canCreate = hasAnyPermission([MenuPermissions.MasterData.ReceiptTypes.Create]);

  const [items] = useState<ReceiptType[]>(mockTypes);
  ...
  if (!canView) {
    return (
      <MainLayout>
        <Head>
          <title>{locale === 'ar' ? 'أنواع سندات القبض - SLMS' : 'Receipt Types - SLMS'}</title>
        </Head>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-green-100 dark:bg-green-900/30 rounded-lg">
              <BanknotesIcon className="h-6 w-6 text-green-600 dark:text-green-300" />
            </div>
            <div>
              <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
                {locale === 'ar' ? 'أنواع سندات القبض' : 'Receipt Types'}
              </h1>
              <p className="text-sm text-gray-500 dark:text-gray-400">
                {locale === 'ar' ? 'تصنيف سندات القبض' : 'Receipt voucher classification'}
              </p>
            </div>
          </div>
