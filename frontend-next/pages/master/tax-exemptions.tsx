...
import { ShieldCheckIcon, EyeIcon, PlusIcon } from '@heroicons/react/24/outline';
...
export default function TaxExemptionsPage() {
  ...
  const [items] = useState<TaxExemption[]>(mockExemptions);
  const [search, setSearch] = useState('');
  const [selected, setSelected] = useState<TaxExemption | null>(null);
  ...
  if (!canView) {
    return (
      <MainLayout>
        <Head>
          <title>{locale === 'ar' ? 'الإعفاءات الضريبية - SLMS' : 'Tax Exemptions - SLMS'}</title>
        </Head>
  ...
  return (
    <MainLayout>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
                {locale === 'ar' ? 'الإعفاءات الضريبية' : 'Tax Exemptions'}
              </h1>
              <p className="text-sm text-gray-500 dark:text-gray-400">
                {locale === 'ar' ? 'أسباب إعفاء ضريبي' : 'Tax exemption reasons'}
              </p>
            </div>
          </div>
      ...
      <Modal
        ...
        title={locale === 'ar' ? 'إعفاء جديد' : 'New Exemption'}
        size="md"
      >
