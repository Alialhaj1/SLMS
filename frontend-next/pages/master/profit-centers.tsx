...
type ProfitCenterStatus = 'active' | 'inactive';
...
const mockProfitCenters: ProfitCenter[] = [
  { id: 1, code: 'PC-LOG', name: 'Logistics Operations', nameAr: 'عمليات اللوجستيات', manager: 'Omar Al-Harbi', managerAr: 'عمر الحربي', status: 'active' },
  { id: 2, code: 'PC-WHS', name: 'Warehousing', nameAr: 'المستودعات', manager: 'Sara Al-Qahtani', managerAr: 'سارة القحطاني', status: 'active' },
  { id: 3, code: 'PC-CUS', name: 'Customs & Clearance', nameAr: 'الجمارك والتخليص', manager: 'Fahad Al-Salem', managerAr: 'فهد السالم', status: 'inactive' },
];
...
export default function ProfitCentersPage() {
  ...
  const canView = hasAnyPermission([MenuPermissions.Master.View, MenuPermissions.MasterData.ProfitCenters.View]);
  const canCreate = hasAnyPermission([MenuPermissions.Master.Create, MenuPermissions.MasterData.ProfitCenters.Create]);
  const canEdit = hasAnyPermission([MenuPermissions.Master.Edit, MenuPermissions.MasterData.ProfitCenters.Edit]);

  const [items] = useState<ProfitCenter[]>(mockProfitCenters);
  const [selectedStatus, setSelectedStatus] = useState<'all' | ProfitCenterStatus>('all');
  ...
  const [formData, setFormData] = useState({
    ...
    manager: '',
    managerAr: '',
    status: 'active' as ProfitCenterStatus,
  });
  ...
  const getStatusBadge = (status: ProfitCenterStatus) => {
    const styles: Record<ProfitCenterStatus, string> = {
      active: 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300',
      inactive: 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300',
    };
    const labels: Record<ProfitCenterStatus, { en: string; ar: string }> = {
  ...
  const handleCreate = () => {
    ...
    setCreateOpen(false);
    setFormData({ code: '', name: '', nameAr: '', manager: '', managerAr: '', status: 'active' });
  };
  ...
  if (!canView) {
    return (
      <MainLayout>
        <Head>
          <title>{locale === 'ar' ? 'مراكز الربحية - SLMS' : 'Profit Centers - SLMS'}</title>
        </Head>
        <div className="text-center py-12">
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-2">
            {locale === 'ar' ? 'لا تملك صلاحية عرض مراكز الربحية.' : "You don't have permission to view profit centers."}
          </p>
        </div>
      </MainLayout>
    );
  ...
  return (
    <MainLayout>
      ...
      <div className="space-y-6 animate-fade-in">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{locale === 'ar' ? 'مراكز الربحية' : 'Profit Centers'}</h1>
              <p className="text-sm text-gray-500 dark:text-gray-400">{locale === 'ar' ? 'تعريف مراكز الربحية لإعداد التقارير والرقابة' : 'Define profit centers for reporting and control'}</p>
            </div>
          </div>
          {canCreate && (
            <Button onClick={openCreate}>
              <PlusIcon className="h-4 w-4" />
              {locale === 'ar' ? 'مركز جديد' : 'New Center'}
            </Button>
          )}
        </div>
            ...
            <table className="w-full">
              <thead className="bg-gray-50 dark:bg-gray-700">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase">{locale === 'ar' ? 'المدير' : 'Manager'}</th>
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                {filtered.map((i) => (
                  <tr key={i.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                    ...
                    <td className="px-4 py-3 text-gray-500">{locale === 'ar' ? i.managerAr : i.manager}</td>
      ...
      <Modal isOpen={!!selected} onClose={() => setSelected(null)} title={locale === 'ar' ? 'تفاصيل المركز' : 'Center Details'} size="lg">
        {selected && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                <p className="text-xs text-gray-500">{locale === 'ar' ? 'المدير' : 'Manager'}</p>
                <p className="font-medium text-gray-900 dark:text-white">{locale === 'ar' ? selected.managerAr : selected.manager}</p>
              </div>
              <div className="p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
      ...
      <Modal isOpen={canCreate && createOpen} onClose={() => setCreateOpen(false)} title={locale === 'ar' ? 'مركز ربحية جديد' : 'New Profit Center'} size="lg">
        <div className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Input label={locale === 'ar' ? 'الكود' : 'Code'} value={formData.code} onChange={(e) => setFormData({ ...formData, code: e.target.value })} placeholder="PC-..." />
            <div>
            ...
            <Input label={locale === 'ar' ? 'المدير (EN)' : 'Manager (EN)'} value={formData.manager} onChange={(e) => setFormData({ ...formData, manager: e.target.value })} />
            <Input label={locale === 'ar' ? 'المدير (AR)' : 'Manager (AR)'} value={formData.managerAr} onChange={(e) => setFormData({ ...formData, managerAr: e.target.value })} />
          </div>
