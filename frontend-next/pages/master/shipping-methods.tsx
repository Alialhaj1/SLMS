...
import { PaperAirplaneIcon, EyeIcon, PlusIcon } from '@heroicons/react/24/outline';
...
const mockMethods: ShippingMethod[] = [
  { id: 1, code: 'SEA', nameEn: 'Sea Freight', nameAr: 'شحن بحري', isActive: true },
  { id: 2, code: 'AIR', nameEn: 'Air Freight', nameAr: 'شحن جوي', isActive: true },
  { id: 3, code: 'LAND', nameEn: 'Land Transport', nameAr: 'نقل بري', isActive: true },
  { id: 4, code: 'COURIER', nameEn: 'Courier', nameAr: 'ساعي / بريد سريع', isActive: true },
];
...
export default function ShippingMethodsPage() {
  ...
  const canView = hasAnyPermission([MenuPermissions.MasterData.ShippingMethods.View]);
  const canCreate = hasAnyPermission([MenuPermissions.MasterData.ShippingMethods.Create]);

  const [items] = useState<ShippingMethod[]>(mockMethods);
  ...
  if (!canView) {
    return (
      <MainLayout>
        <Head>
          <title>{locale === 'ar' ? 'طرق الشحن - SLMS' : 'Shipping Methods - SLMS'}</title>
        </Head>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-sky-100 dark:bg-sky-900/30 rounded-lg">
              <PaperAirplaneIcon className="h-6 w-6 text-sky-600 dark:text-sky-300" />
            </div>
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">
                {locale === 'ar' ? 'طرق الشحن والنقل' : 'Shipping and transport methods'}
              </p>
            </div>
          </div>
