...
import { PlusIcon, MagnifyingGlassIcon, PencilIcon, TrashIcon, FolderIcon } from '@heroicons/react/24/outline';
...
export default function GroupCategoriesPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await fetch('http://localhost:4000/api/master/group-categories', {
  ...
  return (
    <MainLayout>
      <Head><title>{locale === 'ar' ? 'تصنيفات المجموعات - SLMS' : 'Group Categories - SLMS'}</title></Head>
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
                {locale === 'ar' ? 'تصنيفات المجموعات' : 'Group Categories'}
              </h1>
              <p className="text-sm text-gray-500 dark:text-gray-400">
                {locale === 'ar' ? 'إدارة تصنيفات الأصناف بالشجرة الهرمية' : 'Manage item group categories with tree structure'}
              </p>
            </div>
          </div>
          <button className="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm font-medium">
            <PlusIcon className="h-4 w-4 mr-1" />{locale === 'ar' ? 'إضافة تصنيف' : 'Add Category'}
          </button>
        </div>
          ...
          {loading ? (
            ...
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              <thead className="bg-gray-50 dark:bg-gray-900/50">
                <tr>
                  {['Code', 'Name', 'Parent', 'Sort Order', 'Status', 'Actions'].map(h => (
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                {filtered.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-300">{item.parent || '— Root'}</td>
                    <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-300">{item.sort_order}</td>
