...
import { CreditCardIcon, PlusIcon, EyeIcon } from '@heroicons/react/24/outline';

interface PaymentType { id: number; code: string; nameEn: string; nameAr: string; isActive: boolean; }

const mockTypes: PaymentType[] = [
  { id: 1, code: 'CASH', nameEn: 'Cash', nameAr: 'نقدي', isActive: true },
  { id: 2, code: 'CHEQUE', nameEn: 'Cheque', nameAr: 'شيك', isActive: true },
  { id: 3, code: 'BANK', nameEn: 'Bank Transfer', nameAr: 'تحويل بنكي', isActive: true },
];
...
export default function PaymentTypesPage() {
  ...
  const canView = hasAnyPermission([MenuPermissions.MasterData.PaymentTypes.View]);
  const canCreate = hasAnyPermission([MenuPermissions.MasterData.PaymentTypes.Create]);

  const [items] = useState<PaymentType[]>(mockTypes);
  ...
  if (!canView) {
    return (
      <MainLayout>
        <Head><title>{locale === 'ar' ? 'أنواع سندات الصرف - SLMS' : 'Payment Types - SLMS'}</title></Head>
        <div className="text-center py-12"><CreditCardIcon className="w-16 h-16 mx-auto text-gray-400 mb-4" /><h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100">{locale === 'ar' ? 'غير مصرح' : 'Access Denied'}</h2></div>
      </MainLayout>
    );
  }
    ...
    <MainLayout>
      ...
      <div className="space-y-6 animate-fade-in">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-3"><div className="p-2 bg-green-100 dark:bg-green-900/30 rounded-lg"><CreditCardIcon className="h-6 w-6 text-green-600 dark:text-green-300" /></div><div><h1 className="text-2xl font-bold text-gray-900 dark:text-white">{locale === 'ar' ? 'أنواع سندات الصرف' : 'Payment Types'}</h1><p className="text-sm text-gray-500 dark:text-gray-400">{locale === 'ar' ? 'طرق الدفع والسداد' : 'Payment methods'}</p></div></div>
