...
import { PlusIcon, MagnifyingGlassIcon, PencilIcon, TrashIcon, BanknotesIcon } from '@heroicons/react/24/outline';
...
export default function ExpenseCategoriesPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await fetch('http://localhost:4000/api/master/expense-categories', {
  ...
  const taxBadge = (t: string) => {
    const colors: Record<string, string> = {
      ...
      'Non-deductible': 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300',
    };
  ...
  return (
    <MainLayout>
      <Head><title>{locale === 'ar' ? 'فئات المصروفات - SLMS' : 'Expense Categories - SLMS'}</title></Head>
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
                {locale === 'ar' ? 'فئات مصروفات الشحن' : 'Shipping Expense Categories'}
              </h1>
              <p className="text-sm text-gray-500 dark:text-gray-400">
                {locale === 'ar' ? 'إدارة فئات المصروفات الهرمية مع الحسابات' : 'Manage hierarchical expense categories with account codes'}
              </p>
            </div>
          </div>
          <button className="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm font-medium">
            <PlusIcon className="h-4 w-4 mr-1" />{locale === 'ar' ? 'إضافة فئة' : 'Add Category'}
          </button>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
          {loading ? (
            ...
            <div className="p-8 text-center text-gray-500 dark:text-gray-400">
              <BanknotesIcon className="h-12 w-12 mx-auto mb-2 text-gray-300 dark:text-gray-600" />
              {locale === 'ar' ? 'لا توجد فئات' : 'No expense categories found'}
            </div>
          ) : (
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              <thead className="bg-gray-50 dark:bg-gray-900/50">
                <tr>
                  {['Code', 'Name', 'Parent Category', 'Account Code', 'Tax Treatment', 'Actions'].map(h => (
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                {filtered.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-300">{item.parent_category || '— Root'}</td>
                    <td className="px-4 py-3 text-sm font-mono text-gray-600 dark:text-gray-300">{item.account_code}</td>
