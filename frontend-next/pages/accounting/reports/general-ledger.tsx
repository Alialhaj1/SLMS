 ...
 * =====================================================
 * Displays transaction-level detail for selected account(s)
 * Shows opening balance, running balance, closing balance
 */
...
function GeneralLedgerPage() {
  ...
  const loadAccounts = async () => {
    try {
      ...
      if (excludeZero) params.append('exclude_zero', 'true');

      const response = await apiClient.get(`/api/reports/general-ledger/accounts?${params}`);
              ...
              <label className="flex items-center gap-2">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    // Will retrigger account load
                  }}
                  className="rounded"
                />
                <span className="text-sm text-gray-700 dark:text-gray-300">
                  {t('accounting.generalLedger.excludeZero')}
                </span>
              </label>
            </div>
          </div>
