...
type PrepaidStatus = 'active' | 'completed' | 'cancelled';
...
const mockItems: PrepaidExpense[] = [
  { id: 1, reference: 'PPE-2025-0001', vendor: 'Fleet Insurance Co.', vendorAr: 'شركة تأمين الأسطول', amount: 12000, currency: 'SAR', startDate: '2025-01-01', endDate: '2025-12-31', remainingMonths: 10, status: 'active' },
  { id: 2, reference: 'PPE-2024-0042', vendor: 'Office Lease', vendorAr: 'إيجار المكتب', amount: 24000, currency: 'SAR', startDate: '2024-01-01', endDate: '2024-12-31', remainingMonths: 0, status: 'completed' },
  { id: 3, reference: 'PPE-2025-0007', vendor: 'Software Subscription', vendorAr: 'اشتراك برمجيات', amount: 6000, currency: 'SAR', startDate: '2025-02-01', endDate: '2025-07-31', remainingMonths: 5, status: 'active' },
];
...
export default function PrepaidExpensesPage() {
  ...
  const canView = hasAnyPermission([MenuPermissions.Accounting.PrepaidExpenses.View]);
  const canCreate = hasAnyPermission([MenuPermissions.Accounting.PrepaidExpenses.Create]);
  const canEdit = hasAnyPermission([MenuPermissions.Accounting.PrepaidExpenses.Edit]);

  const [items] = useState<PrepaidExpense[]>(mockItems);
  const [search, setSearch] = useState('');
  const [status, setStatus] = useState<'all' | PrepaidStatus>('all');
  const [selected, setSelected] = useState<PrepaidExpense | null>(null);
  ...
  const statusBadge = (s: PrepaidStatus) => {
    const styles: Record<PrepaidStatus, string> = {
      active: 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300',
      completed: 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300',
      cancelled: 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300',
    };
    const labels: Record<PrepaidStatus, { en: string; ar: string }> = {
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((i) => {
      ...
      const qOk =
        ...
        i.vendorAr.toLowerCase().includes(q);
      return sOk && qOk;
    });
  }, [items, search, status]);
  ...
  const handleCreate = () => {
    ...
    setCreateOpen(false);
    setFormData({ reference: '', vendor: '', vendorAr: '', amount: '0', currency: 'SAR', startDate: '', endDate: '' });
  };
  ...
  if (!canView) {
    return (
      <MainLayout>
        <Head>
          <title>{locale === 'ar' ? 'المصروفات المدفوعة مقدماً - SLMS' : 'Prepaid Expenses - SLMS'}</title>
        </Head>
        <div className="text-center py-12">
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-2">{locale === 'ar' ? 'لا تملك صلاحية عرض المصروفات المدفوعة مقدماً.' : "You don't have permission to view prepaid expenses."}</p>
        </div>
      </MainLayout>
    );
  }
  ...
  return (
    <MainLayout>
      ...
      <div className="space-y-6 animate-fade-in">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{locale === 'ar' ? 'المصروفات المدفوعة مقدماً' : 'Prepaid Expenses'}</h1>
              <p className="text-sm text-gray-500 dark:text-gray-400">{locale === 'ar' ? 'متابعة المصروفات التي يتم استهلاكها على فترات' : 'Track expenses amortized over time'}</p>
            </div>
          </div>
          {canCreate && (
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
          <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex flex-wrap items-center justify-between gap-3">
            <div className="flex flex-wrap items-center gap-3">
              <div className="w-80">
                <Input label={locale === 'ar' ? 'بحث' : 'Search'} value={search} onChange={(e) => setSearch(e.target.value)} placeholder={locale === 'ar' ? 'بحث بالمرجع أو المورد...' : 'Search by reference or vendor...'} />
              </div>
              <div>
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                {filtered.map((i) => (
                  <tr key={i.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                    ...
                    <td className="px-4 py-3 text-gray-900 dark:text-white">{locale === 'ar' ? i.vendorAr : i.vendor}</td>
        ...
        {selected && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                <p className="text-xs text-gray-500">{locale === 'ar' ? 'من' : 'From'}</p>
                <p className="font-medium text-gray-900 dark:text-white">{selected.startDate}</p>
              </div>
              <div className="p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                <p className="text-xs text-gray-500">{locale === 'ar' ? 'إلى' : 'To'}</p>
      ...
      <Modal isOpen={canCreate && createOpen} onClose={() => setCreateOpen(false)} title={locale === 'ar' ? 'قيد مصروف مدفوع مقدماً' : 'New Prepaid Expense'} size="lg">
        <div className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Input label={locale === 'ar' ? 'المرجع' : 'Reference'} value={formData.reference} onChange={(e) => setFormData({ ...formData, reference: e.target.value })} placeholder="PPE-2025-0009" />
