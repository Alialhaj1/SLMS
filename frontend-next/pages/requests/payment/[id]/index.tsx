/**
 * Payment Request Detail Page - تفاصيل طلب السداد
 * ======================================================
...
export default function PaymentRequestDetailPage() {
  ...
  const canSubmit = hasAnyPermission(['payment_requests:submit', 'payment_requests:manage']);
  const canApprove = hasAnyPermission(['payment_requests:approve', 'payment_requests:manage']);
  const canReview = hasAnyPermission(['payment_requests:review', 'payment_requests:approve', 'payment_requests:manage']);
  const canExecute = hasAnyPermission(['payment_requests:execute', 'payment_requests:manage']);
  ...
  const handlePrint = () => {
    router.push(`/requests/payment/${id}/print`);
  };
  ...
  const callAction = async (action: 'submit' | 'review' | 'approve' | 'reject' | 'execute', body?: object) => {
    ...
    try {
      ...
      showToast({ type: 'success', message: isArabic
        ? action === 'submit' ? 'تم إرسال الطلب للاعتماد' : action === 'review' ? 'تمت مراجعة الطلب' : action === 'approve' ? 'تمت الموافقة على الطلب' : action === 'reject' ? 'تم رفض الطلب' : 'تم تنفيذ السداد'
        : action === 'submit' ? 'Submitted for approval' : action === 'review' ? 'Request reviewed' : action === 'approve' ? 'Payment approved' : action === 'reject' ? 'Payment rejected' : 'Payment executed'
      });
      // Reload
      const newRes = await fetch(`/api/payment-requests/${id}`, {
        headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
      });
  ...
  const getPaymentMethodLabel = (method: string) => {
    const methods: Record<string, { en: string; ar: string }> = {
      ...
      wire: { en: 'Wire Transfer', ar: 'حوالة سلكية' },
    };
  ...
  return (
    <MainLayout>
      <Head>
        <title>{request.request_number} - {isArabic ? 'طلب سداد' : 'Payment Request'} - SLMS</title>
      </Head>
      ...
      <div className="p-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-600 dark:text-gray-400">
                {isArabic ? 'طلب سداد' : 'Payment Request'}
              </p>
            </div>
          </div>
          <div className="flex gap-2 flex-wrap">
            ...
            {request.status_code === 'DRAFT' && (isOwner || canSubmit) && (
            ...
            {request.status_code === 'SUBMITTED' && canReview && (
              <Button
                variant="primary"
            ...
            {(request.status_code === 'SUBMITTED' || request.status_code === 'REVIEWED') && canApprove && (
              <Button
                variant="primary"
            ...
            {request.status_code === 'APPROVED' && canExecute && (
              <Button
                variant="primary"
              ...
              >
                ...
                {isArabic ? 'تنفيذ السداد' : 'Execute Payment'}
              </Button>
            )}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-indigo-900/20 dark:to-purple-900/20 rounded-lg border border-indigo-200 dark:border-indigo-800 p-6">
              <div className="flex items-center gap-3 mb-2">
                <CreditCardIcon className="w-8 h-8 text-indigo-600 dark:text-indigo-400" />
                <h2 className="text-lg font-semibold text-gray-900 dark:text-white">
                  {isArabic ? 'مبلغ السداد' : 'Payment Amount'}
                </h2>
              </div>
              <div className="text-3xl font-bold text-indigo-600 dark:text-indigo-400">
                {formatAmount(request.payment_amount)} {request.currency_code}
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h2 className="text-lg font-semibold text-gray-900 dark:text-white">
                  {isArabic ? 'تفاصيل السداد' : 'Payment Details'}
                </h2>
              </div>
              <dl className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                ...
                {request.cheque_number && (
                  <div>
                    <dt className="text-sm font-medium text-gray-500 dark:text-gray-400">
                      {isArabic ? 'رقم الشيك' : 'Cheque Number'}
                    </dt>
                    <dd className="mt-1 text-sm text-gray-900 dark:text-white font-mono">
                      {request.cheque_number}
                    </dd>
          ...
          <div className="space-y-6">
            ...
            {request.transfer_request_number && (
              <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800 p-4">
                <h3 className="text-sm font-medium text-blue-800 dark:text-blue-300 mb-2">
                  {isArabic ? 'طلب التحويل المرتبط' : 'Linked Transfer Request'}
                </h3>
            ...
            {request.expense_request_number && (
              <div className="bg-amber-50 dark:bg-amber-900/20 rounded-lg border border-amber-200 dark:border-amber-800 p-4">
                <h3 className="text-sm font-medium text-amber-800 dark:text-amber-300 mb-2">
                  {isArabic ? 'طلب المصروف المرتبط' : 'Linked Expense Request'}
                </h3>
            ...
            {request.executed_by_name && (
              <div className="bg-emerald-50 dark:bg-emerald-900/20 rounded-lg border border-emerald-200 dark:border-emerald-800 p-4">
                <h3 className="text-sm font-medium text-emerald-800 dark:text-emerald-300 mb-2">
                  {isArabic ? 'تم التنفيذ بواسطة' : 'Executed By'}
                </h3>
                <p className="text-sm text-emerald-700 dark:text-emerald-400">{request.executed_by_name}</p>
                {request.executed_at && (
                  <p className="text-xs text-emerald-600 dark:text-emerald-500 mt-1">{formatDate(request.executed_at)}</p>
                )}
              </div>
            )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 max-w-md w-full mx-4">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">
              {isArabic ? 'رفض طلب السداد' : 'Reject Payment Request'}
            </h3>
